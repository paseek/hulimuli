...
class Advertising {
    ...
    public function render_advertise_field( $args = array() ) {
        ...
        if ( $type == 'after_p' ) {
            echo '<div style="margin-bottom: 8px;">Показывать после <input name="' . $name_num . '" type="number" value="' . esc_attr( $value_num ) . '" style="width: 50px"> абзаца. Если задано 0 — не показывается.</div>';
        }

        if ( $type == 'regular' ) {
            echo '<div style="margin-bottom: 25px;"> Выводить: &nbsp;';
            echo '<label style="margin-right: 15px;"><input type="checkbox" name="' . $name_visible . '[]" value="home"'; echo ( in_array('home', $value_visible) ) ? ' checked' : ''; echo '> на главной</label>';
            echo '<label style="margin-right: 15px;"><input type="checkbox" name="' . $name_visible . '[]" value="post"'; echo ( in_array('post', $value_visible) ) ? ' checked' : ''; echo '> в записях</label>';
            echo '<label style="margin-right: 15px;"><input type="checkbox" name="' . $name_visible . '[]" value="page"'; echo ( in_array('page', $value_visible) ) ? ' checked' : ''; echo '> на страницах</label>';
            echo '<label style="margin-right: 15px;"><input type="checkbox" name="' . $name_visible . '[]" value="archive"'; echo ( in_array('archive', $value_visible) ) ? ' checked' : ''; echo '> в архивах</label>';
            echo '<label style="margin-right: 15px;"><input type="checkbox" name="' . $name_visible . '[]" value="search"'; echo ( in_array('search', $value_visible) ) ? ' checked' : ''; echo '> в поиске</label>';
            echo '</div>';
        }

        echo '<div style="margin-bottom: 12px;"><span class="wpshop-rk-settings-label">Минимум символов</span> <input name="' . $name_symbols . '" type="number" min="0" value="' . esc_attr( $value_symbols ) . '" style="width: 100px"> в статье, чтобы показывать рекламу</div>';
        echo '<div style="margin-bottom: 12px;"><span class="wpshop-rk-settings-label">Показывать через</span> <input name="' . $name_days . '" type="number" min="0" value="' . esc_attr( $value_days ) . '" style="width: 50px"> дней после публикации записи. 0 — показывать сразу</div>';
        echo '<div style="margin-bottom: 12px;"><span class="wpshop-rk-settings-label">Не показывать в</span> <input name="' . $name_exclude. '" type="text" value="' . esc_attr( $value_exclude ) . '" > (укажите ID постов через запятую)</div>';
        echo '<div style="margin-bottom: 12px;"><span class="wpshop-rk-settings-label">Показывать только в</span> <input name="' . $name_include. '" type="text" value="' . esc_attr( $value_include ) . '" > (укажите ID постов через запятую)</div>';
        echo '<div style="margin-bottom: 25px;"><span class="wpshop-rk-settings-label">Не показывать в</span> <input name="' . $name_category_exclude. '" type="text" value="' . esc_attr( $value_category_exclude ) . '" > (укажите ID рубрик через запятую)</div>';
    ...
    public function show_ad( $id = '', $post = null ) {
        ...
        if ( is_singular() ) {
            ...
            $show_ad = $this->do_show_ad( $post->ID, $exclude, $include, $category_exclude );
     ...
     * @return bool
     */
    public function do_show_ad( $post_id, $exclude_ids = '', $include_ids = '', $category_ids = '' ) {

        // если значения пустые - вернем true
        if ( empty( $exclude_ids ) && empty( $include_ids ) && empty( $category_ids ) )
            return true;

        if( ! empty( $include_ids ) ) {
            $include_ids = is_string( $include_ids ) ? explode( ',', $include_ids ) : array();
            $show = in_array( $post_id, $include_ids );
        }
        else {
            $exclude_ids = is_string( $exclude_ids ) ? explode( ',', $exclude_ids ) : array();
            $category_ids = is_string( $category_ids ) ? explode( ',', $category_ids ) : array();
            $show = ! in_array( $post_id, $exclude_ids ) && ! in_category( $category_ids, $post_id );
        }
